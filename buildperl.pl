...
my %config = (
  ...
  morebits    => {
	           config_args => '-des -Dusemorebits',
		 },
  "64bitall"  => {
	           config_args => '-des -Duse64bitall',
		 },
  "64bitint"  => {
	           config_args => '-des -Duse64bitint',
		 },
);
...
# run_or_die("sed -i -e '/^.*<builtin>/d' -e '/^.*<built-in>/d' -e '/^.*<command line>/d' -e '/^.*<command-line>/d' makefile x2p/makefile");
  run("perl -ni -we '/<(?:built-?in|command[- _]?line)/ or print' Makefile makefile x2p/Makefile x2p/makefile");
  run_or_die("make -j$opt{jobs} all");
  run("TEST_JOBS=$opt{jobs} make -j$opt{jobs} test_harness") if $opt{test};
  if ($opt{install}) {
...
# run_or_die("sed -i -e 's/<db.h>/<db$ver\\/db.h>/' ext/DB_File/DB_File.xs");
  run("perl -pi -e's/<db.h>/<db$ver\\/db.h>/' ext/DB_File/DB_File.xs");
}
